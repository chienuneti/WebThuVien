...
import { Discipline, Faculty, License, Submission } from '../../models/book.model';
...
export class SubmitDocumentComponent implements OnInit {
  ...
  loadFaculties() {
    this.documentService.getFaculties().subscribe({
      next: (res: any) => {
        this.availableFaculties = Array.isArray(res) ? res : (res.data || []);
      }
    });
  }
  ...
  loadDisciplines() {
    this.documentService.getDisciplines().subscribe({
      next: (res: any) => {
        this.availableDiscipline = Array.isArray(res) ? res : (res.data || []);
      }
    });
  }
  ...
  loadAllowedCollections() {
    this.isLoading = true;
    this.documentService.getAllowedCollections().subscribe({
      next: (res: any) => {
        this.collections = Array.isArray(res) ? res : (res.data || []);

        if (this.collections.length === 0) {
          alert('Bạn không có quyền nộp tài liệu vào bất kỳ bộ sưu tập nào!');
        }

        if (!this.isReviseMode && this.collections.length > 0 && !this.formData.collectionId) {
          this.formData.collectionId = this.collections[0].id;
        }
        this.isLoading = false;
      },
      error: (err) => {
