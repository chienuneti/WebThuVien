...
export class HomeComponent implements OnInit {
    ...
    forkJoin({
      ...
      next: (res) => {
        ...
        if (allResult && allResult.data) {
          this.newDocs = allResult.data.slice(0, 5).map((doc: any) => ({
            ...doc,
            coverPath: doc.coverPath
              ? environment.staticUrl + "/" + doc.coverPath
        ...
        this.trendingDocs = Array.isArray(res.trending)
          ? res.trending.slice(0, 5).map((doc: any) => ({
            ...doc,
            coverPath: doc.coverPath
              ? environment.staticUrl + "/" + doc.coverPath
              : '/assets/images/default-cover.png'
        ...
        this.popularDocs = Array.isArray(res.popular)
          ? res.popular.slice(0, 5).map((doc: any) => ({
            ...doc,
            coverPath: doc.coverPath
              ? environment.staticUrl + "/" + doc.coverPath
              : '/assets/images/default-cover.png'
  ...
  onImgError(event: any) {
    event.target.src = '/assets/images/default-cover.png';
  }
}